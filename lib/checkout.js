...
core_1.default.debug(`clean = ${clean}`);
// Submodules
let submodules = core_1.default.getInput('submodules');
// Fetch depth
let fetchDepth = Math.floor(Number(core_1.default.getInput('fetch-depth')));
if (isNaN(fetchDepth) || fetchDepth < 0) {
    fetchDepth = 0;
}
core_1.default.debug(`fetch depth = ${fetchDepth}`);
// LFS
let lfs = false;
if ((core_1.default.getInput('lfs') || '').toUpperCase() == 'TRUE') {
    lfs = true;
}
core_1.default.debug(`lfs = ${lfs}`);
// Access token
let accessToken = core_1.default.getInput('token');
try {
