...
repositoryPath, repositoryOwner, repositoryName, ref, commit, clean, submodules, nestedSubmodules, fetchDepth, lfs, accessToken) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let git = yield gitCommandManager.CreateCommandManager(repositoryPath, lfs);
        // Existing directory
        if (!isNew) {
            let recreate = false;
        ...
        yield removeGitConfig(git, extraHeaderConfigKey);
        ...
        yield git.config(extraHeaderConfigKey, `AUTHORIZATION: basic ${base64Credentials}`);
        // LFS install
        if (lfs) {
        ...
        // Checkout info
        let checkoutInfo = yield getCheckoutInfo(git, ref, commit);
        // LFS fetch
        ...
        // Submodules
        if (submodules) {
            yield git.submoduleSync(nestedSubmodules);
            yield git.submoduleUpdate(fetchDepth, nestedSubmodules);
        }
...
function cleanup() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let git = yield gitCommandManager.CreateCommandManager(repositoryPath, false);
        yield removeGitConfig(git, configKey);
    });
}
